...
public class Digitridoo
{
	...
	public double[] assessAudioQuality(List<Integer> phrases) { // Post-playing analysis.
		...
		return new double[]{truncate(DoubleStream.of(audioQuality).average().getAsDouble(), 2), truncate(DoubleStream.of(creativityIndex).average().getAsDouble(), 2)}; // warning: .average() returns OptionalDouble which can have "no double" -- in that case exception is thrown.
	}
	...
	public String[] colorize(int phrase) { // todo: change int to long to avoid int limits.
		...
		if (phrase * (1 + overtoneDisparity) > (double)Integer.MAX_VALUE / 10000) // Higher than half-of-intmax = rich, deep tone, lower = shallow tone -- todo: same applies as above! Especially here, tone can be categorized in so many ways
