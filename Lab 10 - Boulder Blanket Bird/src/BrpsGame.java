...
public class BrpsGame
{
	...
	public static void main(String[] args) throws InterruptedException {
		...
		BrpsGame game = new BrpsGame( (int) prompt("How many rounds shall we play? (max 15 rounds)", "Error: invalid number.", 1, 15, true), new int[]{0,0});
		...
		boolean debugMode = prompt("Enable debug mode?", "Error: invalid choice.", new String[]{"y", "n"}, false, false).equals("y");
		...
		while (true) {
			if (game.remainingGames > 0) {
				System.out.println(ANSI_YELLOW + "\n\t\t\t\t\t\t\t「  GAME " + (game.totalGames - game.remainingGames + 1) + "  」\t\t");
				...
				game.remainingGames--;
				System.out.println(ANSI_YELLOW + "Game " + (game.totalGames - game.remainingGames) + " finished!\n\n\n" + ANSI_RESET);
	...
	public static <T extends Tuple> String embellishMessage(Set<T> embellims, String setAbbrev, Player target) { // Slightly embellish each win message so they're not always the same.
		...
		switch (setAbbrev) {
			case "ce" -> { // choiceEmbellims (self choice). "You" or "Your opponent"
				filteredEmbellims = embellims.stream()
						.filter(e -> e.getValue(0).equals(target.choice) || e.getValue(0).equals(choices.NONE))
			...
			case "he" -> { // hintEmbellims (oppo choice). "You" or "Your opponent"
				filteredEmbellims = embellims.stream()
						.filter(e -> e.getValue(0).equals(target.opponents.get(0).choice) || e.getValue(0).equals(choices.NONE))
						.map(e -> e.getValue(1).toString())
						.map(s -> targetName.replaceAll("\\[.*?]", "") + " " + s)
						.collect(Collectors.toSet());
			}
		...
		return selectedEmbellim
				.replace("{CHOICE}", String.valueOf(target.choice).toLowerCase())
	...
	@SafeVarargs
	public static <T> Object[] mergeArray(T[]... arrays) { // <+> APM (add entire arrays to array)
		List<T> combinedArray = new ArrayList<>();

		for (T[] array : arrays) {
			combinedArray.addAll(Arrays.asList(array));
		}
	...
	@SafeVarargs
	public static <T> Object[] mergeArray(T[] array, T... items) { // <+> APM (add singular items to array)
		List<T> combinedArray = new ArrayList<>(List.of(array));
		combinedArray.addAll(Arrays.asList(items));
	...
	public void debugStats() {
		System.out.println(ANSI_BLUE + "\t\t\t\tDebug Log for " + LocalDateTime.now());
		System.out.println("=".repeat(50));
		System.out.println("Current game/total games: " +  (totalGames - remainingGames + 1) + "/" + totalGames);
		System.out.println("Total players (CPU/Human): " + players.size() + " players\n");

		for (Player player : players) {
			System.out.println("Player " + (players.indexOf(player) + 1) + " - (" + ((player instanceof CPU) ? "CPU)" : "Human)\n"));
			System.out.println("# of boulders/blankets/birds played: " + player.stats[0] + " boulders/" + player.stats[1] + " blankets/" + player.stats[2] + " birds"); // # of boulders played, # of blankets, # of birds, victories, losses, ties
			System.out.println("Victories/losses/ties: " + player.stats[3] + " wins/" + player.stats[4] + " losses/" + player.stats[5] + " ties");
			System.out.println("W/L ratio: " + (double)player.stats[3] / player.stats[4]);
			System.out.printf("Hint rate: %.2f", player.hintRate);
			System.out.println("\nLast choice made: " + player.choice + "\n");

			System.out.println("Choice cache: " + player.choiceCache.toString().replace("[", "").replace("]", ""));
			System.out.println("State cache: " + player.stateCache.toString().replace("[", "").replace("]",""));
			System.out.println("Opponents: " + player.opponents.toString().replace("[", "").replace("]",""));

			if (player instanceof CPU) {
				System.out.println("Archetype: "  + ((CPU) player).atype);
				System.out.println("Weights (none, boulder, blanket, bird): " + grammaticParse(Arrays.stream(((CPU) player).weights).mapToObj(w -> truncate(w * 100, 2) + "%").toArray(), "and"));

				if (((CPU) player).atype == CPU.archetype.WIKIAN)
					System.out.println("Turns since reroll (wikian): " + ((CPU) player).turnsSinceReroll);
